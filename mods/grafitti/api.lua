...
function g.register_grafitti(name, def)
	...
	for i = 1, parts_count, 1 do
		...
		minetest.register_node(node_name, {
			...
			node_box      = {
				type = "wallmounted",
				wall_top    = {-0.5,  0.49, -0.5,  0.5,  0.50, 0.5},
				wall_bottom = {-0.5, -0.50, -0.5,  0.5, -0.49, 0.5},
				wall_side   = {-0.5, -0.50, -0.5, -0.49, 0.50, 0.5},
			},
			...
			if node_under_def and node_under_def.buildable_to then
				if node_under_def.groups.grafitti then
					...
					minetest.sound_play("grafitti_scrape",
						{pos = pointed_thing.above, max_hear_distance = 4, gain = 1})
				end
				return nil
			end
